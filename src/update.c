...
// this is printing the values before edit happens
...
Ptr_Rolo_Entry copy_entry (entry) Ptr_Rolo_Entry entry;
{
  ...
  for (j = 0; j < N_BASIC_FIELDS - 1; j++) {
    char *freeV;
      set_basic_rolo_field(j,new_entry,(freeV = copystr(get_basic_rolo_field(j,entry))));
free(freeV);
  }

  char *freeV_1;
  set_basic_rolo_field(N_BASIC_FIELDS - 1,new_entry,(freeV_1 = timestring()));
  free(freeV_1);
  ...
  if (n > 0) {
     ...
     for (j = 0; j < n; j++) {
      char *vv;
         set_other_field(j,new_entry,(vv = copystr(get_other_field(j,entry))));
         free(vv);
     }
